...
const initApp = () => {
  ...
  clearItems.addEventListener("click", (event) => {
    ...
    if (list.length) {
      const confirmed = confirm("Es tu sûre de vouloir effacer la liste?");
      if (confirmed) {
        toDoList.clearList();
...
const addClickListenerToCheckbox = (checkbox) => {
  checkbox.addEventListener("click", (event) => {
    ...
    updateScreenReaderConfirmation(removedText, "supprimé de la liste");
    setTimeout(() => {
      refreshThePage();
    }, 1000);
  });
...
const processSubmission = () => {
  ...
  updatePersistentData(toDoList.getList());
  updateScreenReaderConfirmation(newEntryText, "ajouté");
  refreshThePage();
};
